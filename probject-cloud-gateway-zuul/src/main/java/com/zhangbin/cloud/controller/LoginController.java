...
import com.zhangbin.cloud.dto.LoginResp;
...
//		RequestBodyCheckUtils.requesBodyCheck(bindingResult);
		...
		try {
			...
			user.setUserLogo(tbUser.getUserLogo());
			LoginResp loginResp = new LoginResp(jwtToken,user);
			return DtoUtils.isSuccess(CodeEnum.SYSTEM_LOGIN_SUCCESS,loginResp);
		} catch (Exception e) {
			e.printStackTrace();
			return DtoUtils.returnError(CodeEnum.SYSTEM_LOGIN_EXCEPTION);
		}
