...
public class BeanUtil {
	...
	public static <T,B> T copyOwnerAndSuperProperties(B source,Class<T> target){
		try {
			...
			throw new BusinessException(CodeEnum.WEB_SYSTEM_EXCEPTION);
		}
	}
    ...
    public static <T,B> T copyOwnerAndSuperProperties(B source,T target) throws IllegalArgumentException, IllegalAccessException {
        List<Field> voFileds = ReflexUtil.allField(source.getClass());
        List<Field> modelFileds = ReflexUtil.allField(target.getClass());
	 ...
	 * @throws Exception
	 */
	/*public static <T> CommonResponse<T> createResponseBean(Object source, Class<T> clazz){
		return CommonResponseUtil.objectFormat(createBean(source,clazz));
	}*/
}
